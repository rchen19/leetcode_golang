...
From any valid string V, we may split V into two pieces
X and Y such that X + Y (X concatenated with Y) is equal to V.
...
func isValid(S string) bool {
	...
	if strings.Contains(S, "abc") {
		...
		return isValid(S[:i] + S[i+3:len(S)])
	} else {
		return false
	}
}
